...
"""
Family Guardian 360° - Database Manager (SQLiteCloud Only)
Desenvolvido por: João Layon - Desenvolvedor Full Stack
Sistema exclusivo com SQLiteCloud + Ping automático
"""
...
print("☁️ Modo: SQLiteCloud EXCLUSIVO (sem banco local)")
...
def _ping_loop():
    ...
    while _ping_running:
        try:
            ...
            if success:
                elapsed = (datetime.now() - _last_ping_time).total_seconds() if _last_ping_time else 0
                print(f"✅ Ping OK (última: {elapsed:.1f}s atrás)")
            else:
                print("⚠️ Ping falhou")
        ...
        # Espera 30 segundos antes do próximo ping
        time.sleep(30)
...
def start_ping_service():
    ...
    _ping_thread.start()
    print("🚀 Serviço de ping iniciado com sucesso!")
